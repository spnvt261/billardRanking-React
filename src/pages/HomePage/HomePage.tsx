...
const Home: React.FC = () => {
    ...
    const workspace = localStorage.getItem(LOCAL_STORAGE_WORKSPACE);
    if (workspace) {
    ...
    const handleConfirm = () => {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem(LOCAL_STORAGE_WORKSPACE, value);

            setLoading(false);
            setSuccess(true);

            setTimeout(() => {
                // navigate("/rankings", { state: { initialValue: value } });
                // window.location.reload();
                window.location.href = PATHS.RANKINGS;
            }, 500);
        }, 1000);
    };
    ...
    return (
        <div className="home-pages flex justify-center items-center min-h-screen bg-gray-50">
