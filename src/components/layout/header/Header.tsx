...
const Header = () => {
    ...
    const navMenuActive = () => {
        const rect = ulRef.current?.getBoundingClientRect();
        liRefs.current.forEach((li) => {
            if (li && li.id == 'menuIcon' && rect) {
                li.style.color = 'var(--text-navlink-actived-color)';
                const liRect = li.getBoundingClientRect();
                const relativeX = liRect.left - rect.left;
                const width = liRect.width;
                if (blockRef.current) {
    ...
    const navMenuUnActive = () => {
        liRefs.current.forEach((li) => {
            if (li && li.id == 'menuIcon') {
                li.style.color = 'var(--text-navlink-color)';
            }
        });
    }
                ...
                >
                    {
                        visibleLinks.map((item, index) => {
                            ...
                            return (
                                <NavLink
                                    id={item.isHiddenGroup ? 'menuIcon' : ''}
                                    to={item.path}
                                    key={item.label}
                                    ref={setLiRef(index)}
                                    className={({ isActive }) =>
